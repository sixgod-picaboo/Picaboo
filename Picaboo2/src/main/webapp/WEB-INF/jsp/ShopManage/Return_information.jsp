...
<body id="page-top">
	...
	<div id="wrapper">
		...
		<ul class="sidebar navbar-nav">
			...
			<a class="nav-link"
				href="${pageScope.basePath}ShopServlet?op=search"> 
			...
			<li class="nav-item">
				<a class="nav-link" href="BoosServlet?op=alltype"> 
			...
			<li class="nav-item">
				<a class="nav-link"
				href="${pageScope.basePath}BoosServlet?op=return_queryByPage"> 
			...
			<div class="container-fluid">
				...
				<div class="table-responsive">
					...
					<table class="table">
						...
						<tbody>
							<c:if test="${returnlist.data ne null}">
								<c:forEach items="${returnlist.data}" var="return_goods">
									<tr>
										<td>${return_goods.orders_id}</td>
										<td>${return_goods.user_id}</td>
										<td>${return_goods.goods_id}</td>
										<td>${return_goods.goods_price}</td>
										<td>${return_goods.goods_num}</td>
										<td>${return_goods.return_date}</td>
										<td><c:choose>
												<c:when test="${return_goods.goods_state == 0}">
													未发货
												</c:when>
												<c:when test="${return_goods.goods_state == 1}">
													已发货
												</c:when>
												<c:when test="${return_goods.goods_state == 2}">
													已收货
												</c:when>
											</c:choose></td>
										<td>${return_goods.goods_size}</td>
										<td><c:choose>
												<c:when test="${return_goods.goods_state == 0}">
													<button class="btn btn-primary btnagree"
														value="${return_goods.orders_id}" id="modal-863543"
														href="#modal-container-863543" data-toggle="modal">
														同意退货</button>
													<button class="btn btn-danger btnrefuse"
														value="${return_goods.orders_id}">拒绝退货</button>
												</c:when>
												<c:when test="${return_goods.goods_state == 1}">
													<button class="btn btn-secondary" disabled="disabled"
														value="${return_goods.orders_id}" data-toggle="modal">
														已同意退货</button>
												</c:when>
												<c:when test="${return_goods.goods_state == 2}">
													<button class="btn btn-secondary" disabled="disabled"
		...
		$(function() {
			$(".btnagree").click(function() {
				console.log("qrfqwefwef");
				var flag = confirm("是否确定同意退货");
				var orders_id = $(this).parents("tr").find("td").eq(0).text();
				var goods_id = $(this).parents("tr").find("td").eq(2).text();
				var goods_size = $(this).parents("tr").find("td").eq(7).text();
				if (flag) {
					location.href = "BoosServlet?op=return_agree&orders_id=" + orders_id+"&goods_id="+goods_id+"&goods_size="+goods_size;
				}
			});
			
			$(".btnrefuse").click(function(){
				var flag = confirm("是否确定拒绝退货");
				var orders_id = $(this).parents("tr").find("td").eq(0).text();
				var goods_id = $(this).parents("tr").find("td").eq(2).text();
				var goods_size = $(this).parents("tr").find("td").eq(7).text();
				if (flag) {
					location.href = "BoosServlet?op=return_refuse&orders_id=" + orders_id+"&goods_id="+goods_id+"&goods_size="+goods_size;
				}
			});
		...
		layui.use(['laypage', 'layer'], function(){
			...
			//完整功能
			  laypage.render({
			    elem:'pageDiv'
			    ,count:${returnlist.total} ,
			    curr:${returnlist.page}
			   ,limit:${returnlist.pageSize}
			    ...
			    //首次不执行
			      if(!first){
			        //do something
			    	  location.href="BoosServlet?op=return_queryByPage&page="+obj.curr+"&pageSize="+obj.limit+"&strlike"+$("#strlike").val();
			      }
			    }
			  });
