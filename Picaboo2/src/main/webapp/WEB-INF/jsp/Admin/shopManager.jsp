...
<body>
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a href="#" class="active-menu"><i class="fa fa-desktop"></i> 店铺管理</a>
                    </li>
					<li>
						...
						<tbody id="tbody">
							<c:forEach items="${sessionScope.pd.data}" var="shop">
								<tr>
									...
									<td><lable>${shop.shop_state == 1?"可用":"不可用"}</lable></td>
									<td>${shop.shop_level}</td>
									<td>${shop.shop_info}</td>
									<td><button type="button" class="btn btn-danger ban"
											value="${shop.shop_id}"><lable>${shop.shop_state == 1?"禁用":"解除"}</lable></button> <a
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#like").click(function(){
				location.href = "${pageContext.request.contextPath}/AdminServlet?oop=shop&titleLike=" + $("#titleLike").val();
			});
			
			$(".ban").click(function(){
				if(confirm("是否确认执行此操作？")){
					location.href = "${pageContext.request.contextPath}/AdminServlet?oop=shop&op=ban&sid=" + $(this).val();
				}
			});
