...
<%@ page import="com.sixgod.pojo.Goods"%>
<%
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a href="#" class="active-menu"><i class="fa fa-qrcode"></i>商品审核</a>
                    </li>
                </ul>
        ...
        <div id="page-wrapper">
            <div id="page-inner">
				...
				<div class="form-group">
						<label for="titleLike">商品名：</label> <input type="text"
							class="form-control" id="titleLike" name="titleLike"
							value="${sessionScope.titleLike}" placeholder="请输入要查询的商品名">
					</div>
					...
					<table class="table table-striped">
						...
						<tbody id="tbody">
							<c:forEach items="${sessionScope.pd.data}" var="goods">
								<tr>
									<td><input type="checkbox" value="${goods.goods_id}" style="width:20px;height:20px"/></td>
									<td>${goods.goods_name}</td>
									<td>${goods.goods_price}</td>
									<td>${goods.goods_size}</td>
									<td>${goods.gtype_name}</td>
									<td>${goods.goods_info}</td>
									<td><button type="button" class="btn btn-danger yes"
											value="${goods.goods_id}"><lable>通过</lable></button> <a
										style="margin-left: 10px" id="modal-843654"
										href="#modal-container-843654" data-toggle="modal">
									</a></td>
									<td><button type="button" class="btn btn-danger on"
											value="${goods.goods_id}"><lable>不通过</lable></button> <a
										style="margin-left: 10px" id="modal-843654"
										href="#modal-container-843654" data-toggle="modal">
									</a></td>
								</tr>
	 ...
	 ,curr: ${sessionScope.pd.page} //当前页
	   ,limit:${sessionScope.pd.pageSize}//页面显示数
	    ...
	    //首次不执行
	      if(!first){
	        //do something
	    	  location.href="${pageContext.request.contextPath}/AdminServlet?oop=shop&page="+obj.curr+"&pageSize="+obj.limit+"&titleLike="+$("#titleLike").val();
	      }
	    }
	  });
		...
		$(function(){
			...
			$(".on").click(function(){
				if(confirm("是否确认执行此操作？")){
					var size = $(this).parents("tr").find("td").eq(3).text();
					location.href = "${pageContext.request.contextPath}/Review?oop=good&op=on&id=" + $(this).val()+"&size="+size;
				}
			});
			...
			$(".yes").click(function(){
				if(confirm("是否确认执行此操作？")){
					var size = $(this).parents("tr").find("td").eq(3).text();
					location.href = "${pageContext.request.contextPath}/Review?oop=good&op=yes&id=" + $(this).val()+"&size="+size;
				}
			});
			
			//模糊查询
			$("#like").click(function(){
				location.href = "${pageContext.request.contextPath}/Review?oop=good&titleLike=" + $("#titleLike").val();
			});
